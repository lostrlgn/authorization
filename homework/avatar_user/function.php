<?php
    ...
    $add_user = fn(array &$data, string $login, string $password, $image) => 
        $data[$login] = [
    ...
    $register = function() use ($add_user, $file_write, $file_load, $sign_in) {
        ...
        $uploadfile = 'avatar/' . time() . '_' . bin2hex(random_bytes(25)) . pathinfo($_FILES['file']['name'])['extension'];
        ...
        if ($login && $password) {
            ...
            if (! $result) {
                $add_user($data, $login, $password, $uploadfile);
                $file_write($data);
